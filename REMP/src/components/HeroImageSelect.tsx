    ...
    return (
        <div className="w-full max-w-4xl mx-auto p-6 bg-white rounded-xl">
                ...
                {images.map((img, index) => (
                    ...
                    >
                        <img
                            src={img.url}
                            alt={`Media ${index}`}
                            className={`w-full aspect-square object-cover rounded-lg transition-opacity duration-200 ${selectedImageId === img.id ? 'opacity-100' : 'opacity-70'
                                }`}
                        />
                        <div
                            className={`absolute top-1 left-1 w-5 h-5 rounded-sm border-2 ${selectedImageId === img.id ? 'bg-white border-blue-500' : 'border-white'
            ...
            <div className="flex justify-end mt-6 gap-3">
                ...
                <button
                    onClick={() => selectedImageId && onSave(selectedImageId)}

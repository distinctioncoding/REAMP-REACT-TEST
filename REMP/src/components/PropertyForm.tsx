...
const PropertyForm: React.FC<PropertyFormProps> = ({ onClose }) => {
  ...
  const handleChange = (key: keyof PropertyData, value: number | string) => {
    ...
    <form className="space-y-6 text-gray-800 dark:text-white">
      ...
      <div>
        ...
        <input
          ...
          style={{ height: '56px', borderRadius: '7px' }}
        />
      </div>
            ...
            <label key={option.value} className="flex items-center gap-2 cursor-pointer">
              ...
              <div className={`w-6 h-6 rounded-lg flex items-center justify-center transition
                ${formData.saleCategory === option.value
                  ? 'bg-[#1E2A47] text-white border border-[#1E2A47]'
              ...
              {item.key === 'floorArea' ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => handleChange(item.key as keyof PropertyData, Number(e.target.value))}
                ...
                <div className="flex items-center gap-4">
                  <button
                    ...
                    onClick={() => handleChange(item.key as keyof PropertyData, Math.max(0, (formData as any)[item.key] - 1))}
                  >
                    <FiMinus />
                  </button>

                  <span className="min-w-[20px] text-center">{(formData as any)[item.key]}</span>

                  <button
                    type="button"
                    className="w-8 h-8 border rounded flex items-center justify-center"
                    onClick={() => handleChange(item.key as keyof PropertyData, (formData as any)[item.key] + 1)}
                  >
                    <FiPlus />
                  </button>
                </div>
